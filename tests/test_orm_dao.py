...
from test_base import TestBaseDao
from sqlalchemy import select, dialects, Table, Column, union_all, Identity, ForeignKey, Sequence, schema
...
class TestOrmDao(TestBaseDao):
    ...
    def test_update_not_supported(self):
        ...
        # with pytest.raises(Exception) as e_info:
        #     with Session(self.engine) as session:
        #         self.user.fullname = "Daniel Gutman"
        #         self.user.name = "Daniel"
        #         session.flush()
        #         # session.commit() # not doing nothing
        #
        # assert "Parametered queries not supported" in str(e_info.value)